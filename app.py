...
def initialize_data():
    if 'packing_list' not in st.session_state:
        st.session_state.packing_list = {
            ...
            "[공통] 당일 챙길 것": {
                "<여권/결제>": ["여권", "항공권", "여행자보험", "해외결제·외화카드", "일본 교통카드", "eSIM", "엔화 현금"],
                "<카드 지갑>": ["운전면허증", "신용카드"],
                "<티켓/기타>": ["주차권", "손목밴드 (입장권)", "타임테이블", "손수건", "차 열쇠", "애플워치", "애플워치 충전기/배터리", "멀티탭", "스킨/토너 (여행용)", "크림 (여행용)", "면도기 (여분)", "무선 이어폰", "카메라", "메모리카드"]
            },
            "[공통] 당일 착용할 것": {
...
def generate_checklist_image():
    ...
    # Draw footer
    draw.text((width/2, height - 40), "만든이 @sigong © 2026 | Inspired by c86tori | v1.3.0", fill="#505060", font=font_subtitle, anchor="ms")
